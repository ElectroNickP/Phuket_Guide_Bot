...
async def cmd_start(message: types.Message, bot: Bot, **data):
    ...
    try:
        ...
        # IF BOT_MODE is tourist, everyone is a tourist!
        if config.BOT_MODE == "tourist":
            is_staff = False
        else:
            is_staff = (role in staff_roles) or is_admin

        if not is_staff:
            welcome_text = (
                f"🌴 <b>Твой Phuket Buddy</b> — сделаем твой отдых безупречным! 🌊\n\n"
                f"Привет, {message.from_user.first_name}! 👋 Я здесь, чтобы ты не отвлекался от отдыха на мелочи.\n\n"
                f"🛍 <b>BUDDY SHOP</b>\n"
                f"Заказывай напитки и пляжные аксессуары с доставкой прямо к твоему катеру.\n"
                f"• 💳 <b>Плати в рублях</b> через СБП (NSPK).\n"
                f"• 🚀 <b>Доставим на пирс</b> точно ко времени выезда.\n\n"
                f"🆘 <b>ПОДДЕРЖКА</b>\n"
                f"Нужна помощь по туру или есть вопрос? Мы всегда на связи.\n\n"
                f"👇 <b>Заказывай в чате или в онлайн-магазине:</b>"
            )
        else:
            welcome_text = (
                ...
                f"━━━━━━━━━━━━━━━\n"
                f"📋 <b>ЧТО Я УМЕЮ:</b>\n\n"
                f"📅 <b>Расписание</b> — Твоя программа на сегодня и завтра.\n"
                f"🌊 <b>План на море</b> — Лодки, пирсы и списки гостей.\n"
                f"🚐 <b>Наземный план</b> — Все детали по трансферам.\n"
                f"⏰ <b>Умное пробуждение</b> — Напоминания за час до выезда.\n"
                f"🚀 <b>Отчёты</b> — Быстрая сдача утренних и вечерних отчётов.\n"
                f"🆘 <b>Нужна помощь</b> — Экстренная связь с диспетчером.\n"
                f"━━━━━━━━━━━━━━━\n"
                f"Нажми на любую кнопку внизу, чтобы начать. Удачного дня! 🌟"
            )
